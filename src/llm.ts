  ...
  const result = await openai.beta.chat.completions.parse({
    ...
    response_format: zodResponseFormat(
      z.object({
        approved: z
          .boolean()
          .describe('Whether the movie search call is approved by the user.'),
      }),
      'approval'
    ),
    messages: [
      {
        role: 'system',
        content:
          'Determine if the user approved the tool call. If you are not sure, then it is not approved.',
      },
      { role: 'user', content: userMessage },
    ],
  })

  // assume not refusal thus not doing check
